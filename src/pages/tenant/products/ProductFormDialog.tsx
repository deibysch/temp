...
const CompanyFormDialog: React.FC<Props> = ({
  ...
  const loadDiscounts = async () => {
    ...
    try {
      const data = await productsApi.getDiscountsByCompany(editingCompany.id)
      setDiscounts(data)
    } catch (error) {
  ...
  const handleDiscountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingDiscount) {
        await productsApi.updateDiscount(editingDiscount.id, discountData)
      } else {
        await productsApi.createDiscount(discountData)
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await productsApi.deleteDiscount(discountToDelete.id)
      loadDiscounts()
    } catch (error) {
      // Error ya manejado por la API
    } finally {
  ...
  const handleToggleDiscountStatus = async (discountId: number | string, currentStatus: boolean) => {
    try {
      await productsApi.toggleDiscountStatus(discountId, !currentStatus)
      loadDiscounts()
    } catch (error) {
      // Error ya manejado por la API
    }
